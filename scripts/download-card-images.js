...
const CDN_FULL_URL = (id) => `https://images.ygoprodeck.com/images/cards/${id}.jpg`;
const CDN_ART_URL = (id) => `https://images.ygoprodeck.com/images/cards_cropped/${id}.jpg`;
const CDN_SMALL_URL = (id) => `https://images.ygoprodeck.com/images/cards_small/${id}.jpg`;
 ...
 */
async function fetchWithRetry(url, rateLimiter, retries = MAX_RETRIES) {
    let attempt = 0;
 ...
 */
async function saveMiniVariant(inputBuffer, targetPath) {
 ...
 */
function copyFallback(targetPath, variant) {
 ...
 */
async function processCard(card, rateLimiter, force) {
 ...
 */
async function downloadCardBack(rateLimiter) {
    ...
    let buf = await fetchWithRetry('https://ms.yugipedia.com/e/e5/Back-EN.png', rateLimiter);
    if (!buf) {
...
async function main() {
    ...
    const logProgress = (final = false) => {
        ...
        const etaFormatted = estRemainingSec > 60
            ? `${Math.floor(estRemainingSec / 60)}m ${estRemainingSec % 60}s`
            : `${estRemainingSec}s`;
        const progressLine = `\r[PROGRESS] ${completed}/${totalCards} (${percent}%) | ` +

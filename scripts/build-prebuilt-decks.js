...
function resolveCard(nameOrId) {
    if (typeof nameOrId === 'number') {
        const row = getCardByIdStmt.get(nameOrId);
        if (!row)
            throw new Error(`Card ID ${nameOrId} not found in SQLite database!`);
        return nameOrId;
    }
    const exact = getCardByNameStmt.get(nameOrId);
    if (exact)
        return exact.id;
    const like = getCardByLikeStmt.get(`%${nameOrId}%`);
    if (like)
        return like.id;
...
for (const char of charactersData) {
    for (const deck of char.decks) {
        const mainCards = deck.mainCards.map(resolveCard);

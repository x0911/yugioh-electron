...
const ICONIC_ALLOWLIST_IDS = new Set([
...
async function downloadScriptWithRetry(cardId) {
...
async function downloadBatchScripts(cardIds, aliasMap, concurrency = 15) {
    let downloaded = 0;
    let notFound = 0;
    for (let i = 0; i < cardIds.length; i += concurrency) {
        ...
        if ((i + concurrency) % 300 === 0 || i + concurrency >= cardIds.length) {
            console.log(`      [Scripts Progress] Processed ${Math.min(i + concurrency, cardIds.length)} / ${cardIds.length} cards...`);
        }
    }
    return { downloaded, notFound };
}
export async function buildCardPool() {
    console.log('='.repeat(70));
    console.log('  YU-GI-OH! DESKTOP DUEL — CARD POOL FILTER PIPELINE');
    console.log('='.repeat(70));
    // 1. Read Set Code Whitelist
    console.log('\n[1/6] Reading Set Code Whitelist...');
    const whitelistData = JSON.parse(fs.readFileSync(WHITELIST_SETS_PATH, 'utf-8'));
    ...
    const cdbBuffer = await fetchBuffer('https://raw.githubusercontent.com/ProjectIgnis/BabelCDB/master/cards.cdb');
    ...
    const ygoprodeckData = await fetchJson('https://db.ygoprodeck.com/api/v7/cardinfo.php');
    ...
    const rawDatas = rawDb
        .prepare('SELECT d.*, t.name, t.desc, t.str1, t.str2, t.str3, t.str4, t.str5, t.str6, t.str7, t.str8, t.str9, t.str10, t.str11, t.str12, t.str13, t.str14, t.str15, t.str16 FROM datas d JOIN texts t ON d.id = t.id')
        .all();
    const filteredRows = [];
  ...
  `);
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
    const insertAll = outDb.transaction((rows) => {
        for (const r of rows) {
            insertData.run(r.id, r.ot, r.alias, typeof r.setcode === 'bigint' ? Number(r.setcode) : r.setcode, r.type, r.atk, r.def, r.level, r.race, r.attribute, r.category);
            insertText.run(r.id, r.name, r.desc, r.str1 || '', r.str2 || '', r.str3 || '', r.str4 || '', r.str5 || '', r.str6 || '', r.str7 || '', r.str8 || '', r.str9 || '', r.str10 || '', r.str11 || '', r.str12 || '', r.str13 || '', r.str14 || '', r.str15 || '', r.str16 || '');
        }
    });
    ...
    if (fs.existsSync(spikeOfficialDir)) {
        ...
        for (const f of spikeFiles) {
            if (f.endsWith('.lua')) {
                fs.copyFileSync(path.join(spikeOfficialDir, f), path.join(RESOURCES_OFFICIAL_SCRIPTS_DIR, f));
            }
        }
    }
    ...
    console.log(`      ✓ Script sync complete! Downloaded ${result.downloaded} new scripts (${result.notFound} normal/vanilla cards without scripts).`);
    console.log('\n' + '='.repeat(70));

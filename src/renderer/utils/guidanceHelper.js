 ...
 */
export function getActionGuideInfo(boardState, isUserTurn, prompts, selectedCount = 0) {
    // 1. End Phase Hand-Size Cleanup Prompt (§3.4) - ONLY in End Phase ('EP')
    const isEndPhaseCleanup = boardState.currentPhase === 'EP' &&
        prompts.selectCard &&
    ...
    if (prompts.selectCard) {
        ...
        if (isFieldTarget) {
            const isBattleAttackTarget = boardState.currentPhase === 'BP' &&
                prompts.selectCard.selects.every((s) => s.controller !== boardState.userField.playerId && (s.location === 4 || s.location === undefined));

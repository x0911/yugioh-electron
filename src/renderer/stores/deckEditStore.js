...
function createEmptyDeck(id, name = 'New Custom Deck') {
    return {
        id,
        name,
        main: [],
...
export const useDeckEditStore = defineStore('deckEdit', {
    ...
    getters: {
        ...
        filteredCards: (state) => {
            ...
            const { query, kind, subType, attribute, race, level, minAtk, maxAtk, minDef, maxDef, era, sortBy, sortOrder, } = state.filters;
        ...
        mainDeckCount: (state) => state.activeDeck.main.length,
        extraDeckCount: (state) => state.activeDeck.extra.length,
        deckStats(state) {
            let monsters = 0;
            let spells = 0;
            let traps = 0;
        ...
        deckValidity(state) {
            const cardMap = this.cardMap;
            const getCardName = (id) => cardMap.get(id)?.name || `Card #${id}`;
    ...
    actions: {
        async initStore() {
            ...
            try {
                ...
                if (this.cardPool.length > 0) {
                    const defaultCard = this.cardPool.find((c) => c.name === 'Dark Magician') || this.cardPool[0];
        ...
        newDeck(deckName = 'New Custom Deck') {
        ...
        async saveCurrentDeck() {
            ...
            if (!validity.isValid || this.activeDeck.main.length < 40) {
                const errorMsg = this.activeDeck.main.length < 40
        ...
        addCardToDeck(cardId) {
            ...
            if (currentCount >= DECK_LIMITS.MAX_COPIES_PER_CARD) {
                this.showToast(`Cannot add more than ${DECK_LIMITS.MAX_COPIES_PER_CARD} copies of "${card.name}".`, 'warning');
                return false;
            }
            if (card.isExtraDeck) {
                if (this.activeDeck.extra.length >= DECK_LIMITS.MAX_EXTRA) {
                    this.showToast(`Extra Deck is full (maximum ${DECK_LIMITS.MAX_EXTRA} cards).`, 'warning');
                    return false;
                }
        ...
        removeCardFromDeck(cardId, isExtra = false) {
            if (isExtra) {

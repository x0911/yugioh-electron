...
import { getPersistedSettings, savePersistedSettings, getPersistedCustomDecks, savePersistedCustomDeck, deletePersistedCustomDeck, getPersistedActiveDeckId, savePersistedActiveDeckId, } from '../persistence/store.js';
import { loadCharacters, getCharacterById, getRandomDeckForCharacter, } from '../decks/deckLoader.js';
...
export function registerIpcHandlers() {
    ...
    // Duel Handlers
    ipcMain.handle(IPC_CHANNELS.DUEL_NEW, async (_event, options) => {
    ...
    ipcMain.handle(IPC_CHANNELS.DUEL_COMMAND, async (_event, response) => {
    ...
    ipcMain.handle(IPC_CHANNELS.DUEL_SET_AUTOPLAY, async (_event, autoPlay) => {
    ...
    ipcMain.handle(IPC_CHANNELS.DUEL_GET_CARD_NAME, async (_event, code) => {
    ...
    ipcMain.handle(IPC_CHANNELS.DECK_DELETE_CUSTOM_DECK, async (_event, deckId) => {
    ...
    ipcMain.handle(IPC_CHANNELS.DECK_SET_ACTIVE_ID, async (_event, deckId) => {
    ...
    ipcMain.handle(IPC_CHANNELS.DECK_SAVE, async (_event, _deckName, deckData) => {
        if (deckData && typeof deckData === 'object') {
            savePersistedCustomDeck(deckData);
        }
        return true;
    });
    ...
    ipcMain.handle(IPC_CHANNELS.CHARACTERS_GET_BY_ID, async (_event, id) => {
        return getCharacterById(id) || null;
    });
    ipcMain.handle(IPC_CHANNELS.CHARACTERS_GET_RANDOM_DECK, async (_event, characterId) => {

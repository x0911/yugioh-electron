import { OcgMessageType, OcgResponseType, OcgLocation, OcgPosition, OcgRace, OcgAttribute, SelectIdleCMDAction, SelectBattleCMDAction, ocgPositionParse, ocgPhaseString, } from 'ocgcore-wasm';
/**
 ...
 */
export function parseFieldMask(player, fieldMask, count) {
    const places = [];
 ...
 */
export function getAutoResponse(msg) {
    switch (msg.type) {
        ...
        case OcgMessageType.SELECT_SUM: {
            ...
            const search = (idx, currentIndices, currentSum) => {
 ...
 */
export function sanitizeBigInts(obj) {
    ...
    decode(msg) {
        ...
        switch (rawType) {
            ...
            case OcgMessageType.NEW_PHASE: {
                type = 'NEW_PHASE';
                const phaseName = ocgPhaseString.get(msg.phase) ??
                    `PHASE_${msg.phase}`;
                let phaseCode = 'M1';
                const pLower = phaseName.toLowerCase();
                if (pLower.includes('draw'))
                    phaseCode = 'DP';
                else if (pLower.includes('standby'))
                    phaseCode = 'SP';
                else if (pLower.includes('main1'))
                    phaseCode = 'M1';
                else if (pLower.includes('battle') || pLower.includes('damage'))
                    phaseCode = 'BP';
                else if (pLower.includes('main2'))
                    phaseCode = 'M2';
                else if (pLower.includes('end'))
                    phaseCode = 'EP';
            ...
            case OcgMessageType.SHUFFLE_HAND: {
                type = 'SHUFFLE_HAND';
                const p = msg.player;
                const cards = Array.isArray(msg.cards) ? msg.cards.map((c) => Number(c)) : [];
                return {
                    type,
                    rawType,
                    player: p,
            ...
            case OcgMessageType.ANNOUNCE_CARD: {
                ...
                const opcodes = Array.isArray(msg.opcodes) ? msg.opcodes.map((o) => sanitizeBigInts(o)) : [];
                promptData = {
                    player: msg.player,
                    opcodes,
                };
            ...
            case OcgMessageType.ANNOUNCE_NUMBER: {
                ...
                promptData = {
                    player: msg.player,
                    options: Array.isArray(msg.options) ? msg.options.map((o) => sanitizeBigInts(o)) : [],
                };
                return {
                    type,
                    rawType,
